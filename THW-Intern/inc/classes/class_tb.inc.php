...
*
* Funktion:
*			Eine Interface-Klasse für unsere Datenbank!
*			Dies ist das Urobjekt aller TB-Objekte, da sie alle in der Datenbank
*			an einem TB-OBjekt hängen...
*
*			NEU:
*			Eigentlich muss tb_object2 ALLE Eigenschaften bereits haben,
*			auch Userlisten und ähnliches Zeug!! Deswegen werden diese
*			Funktionalitäten nach tb_object2 verlagert!
*
*			Zu implementieren:
...
*
* Bemerkungen:
* 			tb_object enthält jetzt keine load_tb_object-Methode, da wir aus
*			Geschwindigkeitsgründen das in die jeweiligen vererbten bzw. in
...
class tb_object2
{
	...
	*
	* Parameter:
	* - $tb_object_data: Alle Daten für den Eintrag!
	*
	* Bemerkungen:
	*
	* TODO:
	...
	*******************************************************************************/
	function create_tb_object($tb_object_data)
	{
		...
		***************************************************************************/
		...
		/***************************************************************************
		* Query weiterleiten und ausführen:
		***************************************************************************/
		$db->query($sql);
		...
		if ($db->affected_rows() != -1)
		{
			...
			* Insert-ID zurückgeben und fertig!
			***************************************************************************/
	...
	*
	* Funktion:
	* 			Löscht einen Eintrag aus der tb_object-Datenbank
	*
	* Parameter:
	* - $id: Die ID des zu löschenden Objektes
	*
	* Bemerkungen:
	*
	* TODO:
	* - Nach diversen Anhängseln suchen und diese ebenfalls löschen!
	*
	* DONE:
	*
	*******************************************************************************/
	function remove_tb_object($id)
	{
		...
		***************************************************************************/
		...
		{
			/***************************************************************************
			* URGS! Da ist was schiefgelaufen, es wurde nix gelöscht;
	...
	*			Übergebenen Felder geändert!! Anführungszeichen werden NICHT
	*			automatisch gesetzt!!!!!!!!
	*
	* Parameter:
	* - $id: Die ID des zu ändernden Objektes
	...
	*
	* Funktion:
	*			Löscht eine komplette Userliste!
	*
	* Parameter:
	*
	* Bemerkungen:
	...
	*******************************************************************************/
	function drop_userlist($id)
	{
		...
		{
			/***************************************************************************
			* OK, ID ist da, weiter: löschen aller Einträge die in der Userliste
			* vorliegen:
			***************************************************************************/
			$sql = '
					delete from
			...
			***************************************************************************/
			...
			/***************************************************************************
			* Ok, es existieren jetzt keine Beiträge mehr für diese Liste,
			* jetzt können wir die Liste löschen!
			***************************************************************************/
			$sql = '
					delete from
						' . TB_HELFERLISTE . '
			...
			***************************************************************************/
			if ($this->current_userlist[slots])			// Es gibt eine Beschränkung!
			{
				if ($this->current_userlist[number_of_entries] >= $this->current_userlist[slots])
				{
					...
					{
						// Urgs, das müsste man nochmal überarbeiten!!!!!!!!
